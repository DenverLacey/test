// @TODOS:
// - Change error reporting so that the report is printed in the Evaluation's report().
//
...
#define FIXTURE(T) template<> struct Fixture<T>
#define TESTS static constexpr std::initializer_list<Test> __tests__ =
#define TEST(name, ...) { SHOULD_PASS, __VA_ARGS__, name, __FILE__, __LINE__ }
#define TEST_EXPECT(name, expectation, ...) { expectation, __VA_ARGS__, name, __FILE__, __LINE__ }
...
public:
    ...
    void report(const char *test_name, bool test_meets_expectation, const char *test_file, int test_line) const {
        std::cout << test_file << ':' << test_line << ": " << test_name << "... ";
        const char* c;

        switch (_status) {
            case PASSED:
                c = test_meets_expectation ? color::GREEN : color::RED;
                std::cout << c << "Ok!" << color::RESET << std::endl;
                break;
            case INCONCLUSIVE:
            ...
            case FAILED:
                c = test_meets_expectation ? color::GREEN : color::RED;
                std::cout << c << "Failed!" << color::RESET << std::endl;
                break;
...
template<typename F>
void __run_fixture__(Evaluation& evaluation) {
    ...
    for (const Test& test : tests) {
        ...
        test.fn(tester);
        bool expectation_met = tester.status_meets_expectation(test.expectation);

        tester.report(test.name, expectation_met, test.file, test.line);
...
void run_fixtures() {
    ...
    __run_fixtures__<F, Fs...>(evaluation);
